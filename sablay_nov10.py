...
sys.stdout.write("Optimizer : Adam(lr=1e-3)\n")
sys.stdout.flush()
...
dataset = ImageFolderRevised(root = os.path.sep.join([config.DATASET_DATASET_PATH,"Nov10"]))
...
for epoch in range(num_epochs):
    ...
    train_accuracy = 100.0 * correct / float(len(train_indices))
    ...
    val_accuracy = 100.0 * correct / float(len(test_indices))
    stop = time.time()
